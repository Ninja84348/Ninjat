...
bot = telebot.TeleBot('7887346869:AAF1mtxoISI9LozUlPZgiIVqt2xk0vHpn98')
...
#  Join Official Channel @KaliaYtOwner
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Mood ni hai abhi pelhe purchase kar isse:- @KaliaYtOwner."
...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karle bsdk:-  @Ninja2399 ."
...
def clear_logs_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "Please Contact Owner My Hero."
    bot.reply_to(message, response)
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Ohh!! Hero Contact Owner"
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"Kalua start : {target}:{port} for {time}\nSEC Jabtak YE Attack run krrha hai to iske bichme koi Attack nahe Dalna insan"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "Abhi Kalua Chalu hai. Thoda sabar kar pehle Kalua ka khatam ho Jaye tab tu Chodna."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 120:
                response = "Error: Buy From @Ninja2399 For More Time"
            else:
...
def show_help(message):
    help_text ='''
 /chodo :  HATERS KI DUND ME KALUA KA LUND. 
 /rules : RULES PADHLE KAM AYEGA !!.
 /mylogs : SAB LOG HATERS KI CHUDAI DEKHO.
 /plan : SAB KA BAT HAI.
 /myinfo : APNE KALUA KI PLAN DEKHLE.

 To See Admin Commands:
 /admincmd : Shows All Admin Commands
Buy From :- @Ninja2399
Official Channel :- https://t.me/ninjafxt
'''
...
def welcome_start(message):
    ...
    response = f'''Kalua ki Kala Dunya me aapka swagat hai, {user_name}! Sabse acche se bgmi ki maa behen yahi hack karta hai. Kharidne ke liye Kalia Se bat karlo.
...
3. MAKE SURE YOU JOINED https://t.me/KaliaytOfficial OTHERWISE NOT WORK
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "Ohh!! Owner Se Contact Kalo."
